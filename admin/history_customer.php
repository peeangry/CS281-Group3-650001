...
class myPDF extends FPDF{
  function header()
  {
    ...
    $this->Ln(10);
    $this->Cell(276,5,'History Customer',0,0,'C');
...
$check=$_SESSION["uid"];
 $stmt = $db->query("SELECT * FROM orders,products,user_info WHERE user_info.user_id = '$check' ");
   ...
   //   $this->Ln();
    ...
    $this->Cell(50,10,'Bill Time',1,0,'C');
  ...
  function viewTable($db)
  {
	...
	$this->SetFont('Times','',12);
	 ...
	 $allPrice=0;
   $check=$_SESSION["uid"];
	 $std = $db->query("SELECT * FROM orders,products,user_info WHERE orders.product_id = products.product_id AND orders.user_id=user_info.user_id  ");
	 while($data = $std->fetch(PDO::FETCH_OBJ)){
		 
		$this->Cell(30,10,$data->first_name,1,0,'C');
    $this->Cell(30,10,$data->last_name,1,0,'C');
		$this->Cell(50,10,$data->product_title,1,0,'L');
